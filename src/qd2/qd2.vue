...
<template>
  <div class="qd2">
    <h1 class = "text-primary">{{'Share Your Tips!'}}</h1>
			...
			<button class = "btnshare btn-primary btn-lg"  @click="share">我要分享我的冷知识</button>
						...
						<div v-if="item.allshow">
							<div class = "tipinfo">
								...
								<div v-if="item.btnshow">
									<button class = "btn1 btn-success btn-lg" :style="styles1" @click="like(item.id)">哈哈哈哈哈哈有用++</button>
									<button class = "btn2 btn-danger btn-lg" :style="styles2" @click="notlike(item.id)">无聊--</button>
								</div>
							</div>
						</div>
	 				</blockquote>
